m = Map("wireless", "5 GHz Guest 2 (rai2)", "Secondary virtual interface. Note: Channel and Transmit Power are inherited from the Main radio.")

local iface_sec = nil
m.uci:foreach("wireless", "wifi-iface", function(sec)
    if sec.ifname == "rai2" then
        iface_sec = sec[".name"]
    end
end)
...
if not iface_sec then
    ...
    m.uci:set("wireless", iface_sec, "device", "radio1")
    m.uci:set("wireless", iface_sec, "ifname", "rai2")
    m.uci:set("wireless", iface_sec, "network", "lan")
    m.uci:set("wireless", iface_sec, "mode", "ap")
    m.uci:set("wireless", iface_sec, "ssid", "Zyxel_Guest_rai2")
