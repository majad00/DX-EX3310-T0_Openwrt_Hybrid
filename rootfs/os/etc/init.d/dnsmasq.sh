...
TTNET_EDNS=/tmp/TTnetEdns.txt
TTNET_PPPoEPasswd=/tmp/TTnetPPPoEPasswd.txt
...
startEDNS()
{
	...
	local re=$(logchk)
	local pppoePasswd=`cat $TTNET_PPPoEPasswd`
	$EXEC_PATH/$EXEC_NAME -u $USER -q --conf-file=$EXEC_CONF --add-mac --add-ppp-username $pppoePasswd
	echo "$EXEC_PATH/$EXEC_NAME -u $USER -q --conf-file=$EXEC_CONF --add-mac --add-ppp-username $pppoePasswd TTT"
}
...
case "$1" in
	start)
		PID=$(pgrep dnsmasq)
		if [ "$PID" == "" ] ; then
			if [ -f $TTNET_EDNS ] && [ -f $TTNET_PPPoEPasswd ]; then
				startEDNS
			else
		  start
			fi
